  ...
  private footer(): string {
    ...
    if (roster && guild.rank === 'leader') {
      const label =
        roster.nextRosterPrice === null
          ? `<button class="btn ui-btn" data-act="guild-expand" disabled>${esc(t('hudChrome.social.roster.maxed'))}</button>`
          : `<button class="btn ui-btn" data-act="guild-expand">${esc(t('hudChrome.social.roster.expand', { seats: formatNumber(GUILD_ROSTER_PAGE_SEATS, { maximumFractionDigits: 0 }), price: formatMoney(roster.nextRosterPrice) }))}</button>`;
    ...
    foot +=
      guild.rank === 'leader' && guild.members.length > 1
        ? `<div class="soc-add soc-leave"><button class="btn ui-btn ui-btn--red" data-act="guild-disband">${esc(t('hud.social.disbandGuild'))}</button></div>`
        : `<div class="soc-add soc-leave"><button class="btn ui-btn ui-btn--red" data-act="guild-leave">${esc(t('hud.social.leaveGuild'))}</button></div>`;
    return foot;
  }
