...
| Socket | `.ui-socket` `.ui-socket-art` `.ui-socket-key` `.ui-socket-count` `.ui-socket-cd` `.ui-socket-cd-text` | `--socket-size` `--socket-size-bag` `--socket-size-bank` `--socket-size-stance` `--socket-gap` `--socket-row-gap` `--socket-art-inset` `--radius-socket` `--color-socket-rim` `--socket-fill` `--socket-fill-empty` `--shadow-socket` `--glow-gold` `--color-proc-rim` `--color-proc-glow` `--cd-fill` | `.ui-socket--bag` `.ui-socket--bank` `.ui-socket--stance` `.ui-socket.is-empty` `.ui-socket.is-used` `.ui-socket.is-on` `.ui-socket.is-proc` `.ui-socket.is-oor` `.ui-socket.is-unusable` |
