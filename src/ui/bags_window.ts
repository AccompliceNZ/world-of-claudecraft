...
export class BagsWindow {
  ...
  private buildStackCell(
    ...
    const world = this.deps.world();
    {
      ...
      row.className = `bag-item ui-socket ui-socket--bag q-${bagQualityKey(item)}${bagRimClasses(questMark, fineMark)}`;
  ...
  private buildEmptyCell(cell: number | null): HTMLElement {
    const el = document.createElement('div');
    el.className = 'bag-item empty ui-socket ui-socket--bag';
