...
export class LootWindowController {
  ...
  private titleHtml(title: string): string {
    return `<div class="panel-title ui-win-head"><span class="ui-win-title">${esc(title)}</span><button type="button" class="x-btn ui-x-btn" data-close aria-label="${esc(t('itemUi.loot.close'))}">${svgIcon('close')}</button></div>`;
  }
