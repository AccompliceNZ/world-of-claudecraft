...
export class WocMarketWindow {
  ...
  private bidFormHtml(
    ...
    return (
      ...
      `<input type="number" class="ui-input" inputmode="decimal" min="0" step="0.25" data-field="bid-usd" data-focus-key="wm-bid-usd" placeholder="${esc(
  ...
  private confirmFieldsHtml(model: Extract<WocMarketViewModel, { kind: 'ready' }>): string {
    ...
    const termsRow = model.activity?.termsAccepted
      ? ''
      : `<div class="wm-terms-row"><label class="wm-terms"><input type="checkbox" class="ui-check" data-field="accept-terms" data-focus-key="wm-terms" ${this.acceptTerms ? 'checked' : ''} /> ${esc(
